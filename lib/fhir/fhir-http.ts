import type { FhirServerConfig } from './servers';

export type FhirFetchOpts = {
...
export function createFhirClient(getConfig: () => FhirServerConfig, label: string) {
  async function fhirFetch<T>(path: string, opts: FhirFetchOpts = {}): Promise<T> {
    const { baseUrl, bearerToken } = getConfig();
    if (!baseUrl) {
      throw new FhirError(
        0,
        null,
        `${label} base URL is not configured. Check environment variables or Admin → Clinic settings.`,
      );
    }
    ...
    if (!res.ok) {
      ...
      throw new FhirError(res.status, outcome, `${label} ${res.status} on ${path}`);
    }
