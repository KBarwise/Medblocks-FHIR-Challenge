...
export function MeasureTrendsDashboard({
  patientId,
  sectionId,
  title,
  description,
  ...
  defaultSelected,
  exportPrefix,
  emptyMessage = 'Select at least one measure.',
}: {
  patientId: string;
  sectionId: string;
  title: string;
  ...
  return (
    <div id={sectionId} className="space-y-4 scroll-mt-24">
      <Card>
        <CardTitle icon={icon}>{title}</CardTitle>
        <p className="text-[12px] text-ink-500 mb-3">{description}</p>
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          <DateRangePicker value={range} onChange={setRange} />
          <button
            type="button"
            disabled={series.every(s => s.points.length === 0)}
            onClick={() => exportCsv(series, `${exportPrefix}-${patientId}-${range}.csv`)}
      ...
      <Card>
        {codeOptions.length === 0 && (
          <p className="text-[12px] text-ink-500 py-6 text-center">No measures recorded yet.</p>
        )}
        {codeOptions.length > 0 && selectedCodes.length === 0 && (
          <p className="text-[12px] text-ink-500 py-6 text-center">{emptyMessage}</p>
        )}
        ...
        {selectedCodes.length > 0 && !isLoading && !isError && (
          <FhirTrendChart
            key={`${sectionId}-${range}-${selectedCodes.join(',')}`}
            series={series}
